 ...
 * Tela de treinos com dias da semana e exercícios expansíveis.
 */
...
export default function TreinosScreen() {
  ...
  useEffect(() => {
    async function carregar() {
      ...
      setTreinos(data);
      const diaAtual = data.find(t => t.diaSemana === diaSelecionado);
    ...
    <View style={[styles.container, { backgroundColor: tema.background }]}>
      ...
      <View style={styles.diasRow}>
        {DIAS.map(dia => (
          <TouchableOpacity
            key={dia}
            style={[styles.diaBtn, diaSelecionado === dia && styles.diaBtnActive]}
            onPress={() => {
              setDia(dia);
          ...
          >
            <Text style={[styles.diaTxt, diaSelecionado === dia && styles.diaTxtActive]}>
              {dia}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      >
        {!temTreino ? (
          <View style={styles.semTreino}>
            ...
            <Text style={[styles.semTreinoTxt, { color: tema.text }]}>Dia de descanso!</Text>
          ...
          <>
            {treinoDia.grupos.map(grupo => {
              ...
              return (
                <View key={grupo.id} style={[styles.grupoContainer, { backgroundColor: tema.card }]}>
                  <TouchableOpacity
                  ...
                  >
                    <View style={styles.grupoHeaderLeft}>
                      ...
                      <Text style={[styles.grupoNome, { color: tema.text }]}>{grupo.nome}</Text>
                    </View>
                    <Text style={[styles.grupoChevron, { color: tema.subtext }]}>
                      {expandido ? '∧' : '∨'}
                    </Text>
                  </TouchableOpacity>
                    ...
                    <View style={styles.exerciciosContainer}>
                      {grupo.exercicios.map(exercicio => (
                        <View key={exercicio.id} style={styles.exercicioCard}>
                          <View style={[styles.exercicioIconeWrap, { backgroundColor: temaDark ? '#2a2a2a' : HD.cardLight }]}>
                            <Text style={styles.exercicioIcone}>🤸</Text>
                          </View>
                          <View style={styles.exercicioInfo}>
                            <Text style={[styles.exercicioNome, { color: tema.text }]}>
                              {exercicio.nome}
                            </Text>
                            <View style={styles.badgesRow}>
                              <View style={styles.badge}>
                                <Text style={styles.badgeLabel}>Series</Text>
                                <Text style={styles.badgeValue}>{exercicio.series}</Text>
                              </View>
                              <View style={styles.badge}>
                                <Text style={styles.badgeLabel}>Reps</Text>
                                <Text style={styles.badgeValue}>{exercicio.reps}</Text>
                              </View>
                              <View style={styles.badge}>
                                <Text style={styles.badgeLabel}>Intervalo</Text>
                                <Text style={styles.badgeValue}>{exercicio.intervalo}</Text>
                              </View>
                            </View>
                          </View>
                        </View>
            ...
            >
              ...
              <Text style={styles.btnConcluirTxt}>Solicitar novo treino</Text>
            </TouchableOpacity>
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  diaBtn:       { width: 48, height: 48, borderRadius: 24, backgroundColor: HD.dayInactive, alignItems: 'center', justifyContent: 'center' },
  ...
  grupoHeader:     { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 16 },
  ...
  exercicioIconeWrap:  { width: 52, height: 52, borderRadius: 26, alignItems: 'center', justifyContent: 'center' },
  exercicioIcone:      { fontSize: 24 },
  exercicioInfo:       { flex: 1 },
  exercicioNome:       { fontSize: 14, fontWeight: '700', marginBottom: 8 },

  badgesRow:   { flexDirection: 'row', gap: 8 },
  badge:       { backgroundColor: HD.primary, borderRadius: 20, paddingHorizontal: 12, paddingVertical: 4, alignItems: 'center' },
  badgeLabel:  { fontSize: 9, color: HD.white, fontWeight: '600' },
  badgeValue:  { fontSize: 13, color: HD.white, fontWeight: '800' },

  btnConcluir:      { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: HD.primary, borderRadius: 30, paddingVertical: 16, marginTop: 8, gap: 8 },
  btnConcluirIcone: { fontSize: 20, color: HD.white, fontWeight: '700' },
  btnConcluirTxt:   { fontSize: 16, fontWeight: '700', color: HD.white },
});